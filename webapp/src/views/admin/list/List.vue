    ...
    <CCardHeader>
      <CRow>
        <CCol :xs="6" class="mt-2">
          <div><strong>Admin</strong> <small>List</small></div>
        </CCol>
        <CCol :xs="6" class="d-flex justify-content-end">
          <div>
            <CButton color="primary" variant="outline" @click="$router.push({ path: `/admin/create` })">
        ...
        <CTableBody>
          <CTableRow v-for="record in records">
            ...
            <CTableDataCell class="text-end">
              <a class="link me-2" @click="$router.push({ path: `/admin/view/${record.id}` })">
                <CIcon :icon="icons.cilZoomIn" />
              </a>
              <a class="link me-2" @click="$router.push({ path: `/admin/update/${record.id}` })">
...
export default {
  ...
  methods: {
     ...
     */
    getList() {
      api.post('/admin/', this.form).then((res) => {
        if (res) {
